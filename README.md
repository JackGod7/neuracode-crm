...
## Que es Neuracode CRM?

Neuracode CRM es un CRM open-source que se personaliza automaticamente a tu negocio. Abrelo con Claude Code, ejecuta `/setup`, y el CRM se adapta a tu industria, tu pipeline, y tu forma de trabajar. Todo corre en tu computadora — tus datos nunca salen de tu maquina.

## Por que Neuracode CRM?
...
```json
{
  "mcpServers": {
    "neuracode-crm": {
      "command": "npx",
      "args": ["tsx", "/ruta/a/neuracode-crm/mcp/crm-server.ts"]
    }
  }
}
```
...
## What is Neuracode CRM?

Neuracode CRM is an open-source CRM that automatically customizes itself to your business. Open it with Claude Code, run `/setup`, and the CRM adapts to your industry, pipeline, and workflow. Everything runs on your computer — your data never leaves your machine.
...
```json
{
  "mcpServers": {
    "neuracode-crm": {
      "command": "npx",
      "args": ["tsx", "/path/to/neuracode-crm/mcp/crm-server.ts"]
    }
  }
}
```
...
**Neuracode CRM** — Built with Claude Code for the community.
